...
// ║  Flutter 3.41+ / Dart 3+                                    ║
// ║  REQUISITO ANDROID: agregar en AndroidManifest.xml          ║
...
// ════════════════════════════════════════════════════════════════
// CONTROLADOR DE RED  ← FIX PRINCIPAL
//
// Problema anterior: socket.close() sin leer respuesta.
// El firmware descartaba el comando porque el cliente cerraba
// la conexión antes del ACK HTTP.
//
// Fix: HTTP/1.0 + Connection:close → leer hasta EOF → cerrar.
// Rutas Tasmota corregidas: Power+On (no Power%20On).
// Timeout aumentado a 5s.
// ════════════════════════════════════════════════════════════════
class NetCtrl {
...
void showSnack(BuildContext ctx, String msg, {bool error = false}) {
  ...
  ScaffoldMessenger.of(ctx).showSnackBar(
    SnackBar(
      ...
      behavior: SnackBarBehavior.floating,
      shape:
          const RoundedRectangleBorder(borderRadius: R.sm),
...
class DashboardPage extends StatefulWidget {
  ...
  @override
  State<DashboardPage> createState() => _DashState();
}
...
class _DashState extends State<DashboardPage>
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: _DashHeader(
                  enc: enc, total: total, pulse: _pulse),
            ),
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                const _SectionHeader('Acceso rápido por artefacto'),
  ...
  List<Widget> _buildHabitaciones(DispositivosNotifier n) {
    ...
    for (final d in n.items) {
      map.putIfAbsent(d.habitacion, () => []).add(d);
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(_saludo(),
              style:
                  const TextStyle(fontSize: 13, color: C.t2)),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard(
      this.label, this.value, this.icon, this.color, this.glow);
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration:
                  BoxDecoration(color: glow, borderRadius: R.xs),
      ...
      child: Column(
        ...
        children: [
          ...
          Text('$enc de $total dispositivos activos',
              style:
                  const TextStyle(fontSize: 11, color: C.t2)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      children: CatArtefacto.values.map((cat) {
        ...
        final enc =
            deEsaCat.where((d) => d.encendido).length;
        final hasActive = enc > 0;
        final col = cat.catColor;
        return Container(
          decoration: BoxDecoration(
            color: hasActive ? col.withOpacity(0.14) : C.card,
            borderRadius: R.sm,
            border: Border.all(
              color: hasActive
            ...
            children: [
              ...
              Text(cat.label,
                  ...
                  style: TextStyle(
                      ...
                      fontWeight: FontWeight.w600,
                      color: hasActive ? col : C.t3)),
              if (deEsaCat.isNotEmpty)
                Text('$enc/${deEsaCat.length}',
                    style: TextStyle(
                        fontSize: 8,
                        color: hasActive ? col : C.t3)),
            ],
...
class _HabCardCompactState extends State<_HabCardCompact> {
  ...
  IconData _habIcon(String n) {
    ...
    if (l.contains('dorm') ||
        l.contains('cuarto') ||
        l.contains('habit')) return Icons.bed_rounded;
    if (l.contains('garaje')) return Icons.garage_rounded;
    if (l.contains('jardín') ||
        l.contains('jardin') ||
        l.contains('patio')) return Icons.yard_rounded;
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        if (_exp) ...[
          ...
          ...widget.dispositivos.map((d) => _MiniTile(
                d: d,
...
class _ChipCount extends StatelessWidget {
  final int enc, total;
  const _ChipCount({required this.enc, required this.total});

  @override
  Widget build(BuildContext context) {
    final c =
        enc == 0 ? C.t3 : enc == total ? C.green : C.orange;
    return Container(
      padding:
          const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
      decoration: BoxDecoration(
          color: c.withOpacity(0.15), borderRadius: R.xl),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 6),
          const Text(
            'Agrega dispositivos en la pestaña\nDispositivos',
            textAlign: TextAlign.center,
...
class _DevTileState extends State<_DevTile>
  ...
  Future<void> _doToggle() async {
    ...
    if (mounted) setState(() => _busy = false);
    if (!ok) {
      widget.onSnack('Error al controlar ${widget.d.nombre}',
          error: true);
    }
  }

  void _confirmDelete(BuildContext ctx, Dispositivo d) {
    showDialog(
      context: ctx,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 3),
                    Row(children: [
                      _MicroBadge(d.tipo.label,
                          col: col),
                      const SizedBox(width: 6),
                      _MicroBadge(d.habitacion,
                          col: C.t2),
                    ]),
                  ],
                ),
...
class _AddFormState extends State<_AddForm> {
  ...
  String _rutaInfo() {
    switch (_tipo) {
      case TipoD.tasmota:
        return 'Tasmota: GET /cm?cmnd=Power+On | /cm?cmnd=Power+Off\n'
            'Requiere Webserver activo en el dispositivo.';
      case TipoD.sonoff:
        return 'Sonoff DIY/Custom: GET /control?cmd=on | /control?cmd=off\n'
            'Requiere firmware personalizado o Tasmota.';
      case TipoD.shelly:
        return 'Shelly Gen1: GET /relay/0?turn=on | /relay/0?turn=off\n'
            'Verifica en http://<IP>/relay/0';
      case TipoD.celular:
        return 'Servidor HTTP local en el celular. Rutas: /on y /off.\n'
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const _Lbl('Nombre del dispositivo'),
              TextFormField(
                controller: _cNombre,
  ...
  Future<void> _doPing() async {
    ...
    final puerto =
        int.tryParse(_cPuerto.text.trim()) ?? 80;
    if (ip.isEmpty) {
      widget.onSnack('Ingresa la IP primero',
          error: true);
      return;
    }
    setState(() {
  ...
  Future<void> _doSave() async {
    ...
    final ok = await widget.notifier.agregar(
      ...
      cat: _cat,
      ip: _cIp.text,
      puerto: int.parse(_cPuerto.text.trim()),
    ...
    if (mounted) {
      ...
      if (ok) {
        Navigator.pop(context);
        widget.onSnack('Dispositivo agregado correctamente');
      } else {
        widget.onSnack(
          'No se pudo conectar.\n'
          'Verifica IP, puerto y que estés en la misma red WiFi.',
          error: true,
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Text('Eliminar dispositivo',
          style:
              TextStyle(color: C.t1, fontSize: 16)),
      content: Text(
