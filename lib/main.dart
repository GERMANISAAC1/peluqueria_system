// ╔══════════════════════════════════════════════════════════════╗
// ║  DOMÓTICA PRO  —  v3.0  Producción                          ║
// ║  Flutter 3.41+  |  Dart 3+                                  ║
// ║  Fixes: HTTP LAN real, lectura de respuesta, rutas correctas ║
// ╚══════════════════════════════════════════════════════════════╝
//
// REQUISITO ANDROID (android/app/src/main/AndroidManifest.xml):
...
class C {
  ...
  static const borderHi   = Color(0xFF2D3D6A);
  // acentos
...
extension TipoDX on TipoD {
  ...
  Color get glow => switch (this) {
    TipoD.tasmota => C.blueGlow,
    TipoD.sonoff  => C.orangeGlow,
    TipoD.shelly  => C.greenGlow,
    TipoD.celular => C.purpleGlow,
    TipoD.otro    => const Color(0x227B8DB8),
  };
  static TipoD fromStr(String s) => TipoD.values.firstWhere(
      (e) => e.name == s.toLowerCase(), orElse: () => TipoD.otro);
}

// Categoría visual del artefacto
...
class Dispositivo {
  ...
  factory Dispositivo.fromJson(Map<String, dynamic> j) => Dispositivo(
    ...
    cat:         CatArtefactoX.fromStr(j['cat'] ?? 'otro'),
    ip:          j['ip'],
    puerto:      j['puerto'] ?? 80,
    ...
    ultimaAccion: j['ultimaAccion'] != null
        ? DateTime.tryParse(j['ultimaAccion']) : null,
  );
...
// ═══════════════════════════════════════════════════════════════
// CONTROLADOR DE RED — HTTP raw sobre TCP  ← FIX PRINCIPAL
//
// Problemas del código anterior:
//   1. socket.close() sin leer respuesta → el firmware descarta la
//      petición porque el cliente cerró antes del ACK HTTP.
//   2. Tasmota usa '+' no '%20' en el query string del cmnd.
//   3. Android API 28+ bloquea cleartext → añadir usesCleartextTraffic.
//   4. Timeout 2s muy corto en WiFi congestionado → 5s.
//
// Solución: escribir la request, luego LEER hasta EOF o timeout,
// después cerrar. Así el firmware completa el ciclo HTTP/1.0.
// ═══════════════════════════════════════════════════════════════
class NetCtrl {
  // Envía GET y ESPERA la respuesta completa antes de cerrar.
  // Retorna el body como String (puede estar vacío si no hay body).
  // Lanza excepción si no se puede conectar.
  static Future<String> _get(String ip, int puerto, String path,
      {Duration timeout = const Duration(seconds: 5)}) async {
    final socket = await Socket.connect(ip, puerto, timeout: timeout);
    try {
      // Usamos HTTP/1.0 para que el servidor cierre tras la respuesta
      // (evita tener que detectar Content-Length o chunked encoding).
      final req =
          'GET $path HTTP/1.0\r\n'
          'Host: $ip\r\n'
      ...
      // Leemos toda la respuesta hasta que el servidor cierre.
  ...
  // ── Rutas correctas por firmware ──────────────────────────────
  // Tasmota: /cm?cmnd=Power+On  ('+' es el estándar application/x-www-form-urlencoded)
  // Sonoff DIY API v2: /zeroconf/switch  (requiere POST, no GET)
  //   → Pero la mayoría de Sonoff con firmware custom usan /control?cmd=on
  // Shelly Gen1: /relay/0?turn=on
  // Shelly Gen2/3: /rpc/Switch.Set?id=0&on=true
  // Celular (servidor HTTP local): /on  /off
  // Genérico: /on  /off

  static String _rutaOn(Dispositivo d) => switch (d.tipo) {
  ...
  static String _rutaOff(Dispositivo d) => switch (d.tipo) {
  ...
  // Encender — retorna true si HTTP respondió (no importa el código)
  static Future<bool> encender(Dispositivo d) async {
    try {
      await _get(d.ip, d.puerto, _rutaOn(d));
      return true;
    } catch (e) {
  ...
  static Future<bool> apagar(Dispositivo d) async {
    try {
      await _get(d.ip, d.puerto, _rutaOff(d));
      return true;
    } catch (e) {
  ...
  // Ping TCP puro (no HTTP) — para verificar que el puerto está abierto
  static Future<bool> ping(String ip, int puerto,
  ...
  // Consulta estado actual (solo Tasmota por ahora)
  static Future<bool?> consultarEstado(Dispositivo d) async {
    ...
    try {
      ...
      // Tasmota responde: {"POWER":"ON"} o {"POWER":"OFF"}
...
class DispositivoRepo {
  ...
  static void guardar(SharedPreferences p, List<Dispositivo> items) =>
      p.setString(_key, jsonEncode(items.map((d) => d.toJson()).toList()));
}
...
class DispositivosNotifier extends ChangeNotifier {
  ...
  Set<String> get habitaciones {
    final set = _items.map((d) => d.habitacion).toSet();
  ...
  Future<void> toggleTodos(bool encender) async {
    final targets = _items
        ...
        .map((d) => d.id)
        .toList();
    for (final id in targets) await toggle(id);
  }

  // Sincroniza el estado real del dispositivo (solo Tasmota)
  Future<void> sincronizar(int id) async {
    final idx = _items.indexWhere((d) => d.id == id);
    if (idx == -1 || _demo) return;
    final estado = await NetCtrl.consultarEstado(_items[idx]);
    if (estado != null && estado != _items[idx].encendido) {
      _items[idx] = _items[idx].copyWith(encendido: estado);
      notifyListeners();
      _save();
    }
  }
...
class DomoticaApp extends StatelessWidget {
  ...
  ThemeData _theme() => ThemeData(
    ...
    textButtonTheme: TextButtonThemeData(
        style: TextButton.styleFrom(foregroundColor: C.t2)),
...
class _DashState extends State<DashboardPage> with SingleTickerProviderStateMixin {
      ...
      return CustomScrollView(slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverList(delegate: SliverChildListDelegate([
            ...
            _SectionHeader('Acceso rápido por artefacto'),
            const SizedBox(height: 12),
            _CatGrid(notifier: n),
            const SizedBox(height: 22),

            // ── Habitaciones preview ─────────────────────────
            _SectionHeader('Habitaciones'),
            const SizedBox(height: 12),
            if (n.items.isEmpty)
              _EmptyState(onAdd: () {})
            else
              ..._buildHabitaciones(n),

            // ── Log rápido ───────────────────────────────────
            if (n.log.isNotEmpty) ...[
              const SizedBox(height: 22),
              _SectionHeader('Última actividad'),
  ...
  List<Widget> _buildHabitaciones(DispositivosNotifier n) {
    final map = <String, List<Dispositivo>>{};
    for (final d in n.items) map.putIfAbsent(d.habitacion, () => []).add(d);
    return map.entries.map((entry) =>
      _HabCardCompact(
        nombre: entry.key,
        dispositivos: entry.value,
        notifier: n,
        onSnack: (msg, {error = false}) => showSnack(context, msg, error: error),
      ),
    ).toList();
  }
}
...
class _DashHeader extends StatelessWidget {
  ...
  final int enc, total;
  const _DashHeader({required this.notifier, required this.pulse, required this.enc, required this.total});
  ...
  Widget build(BuildContext ctx) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.end, children: [
      Text(_saludo(), style: const TextStyle(fontSize: 13, color: C.t2)),
  ...
  Widget build(BuildContext ctx) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('DEMO', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800,
...
// Grid de categorías de artefactos
class _CatGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext ctx) {
    final cats = CatArtefacto.values;
    return GridView.count(
      ...
      children: cats.map((cat) {
        final deEsaCat = notifier.items.where((d) => d.cat == cat).toList();
        final enc = deEsaCat.where((d) => d.encendido).length;
        final hasActive = enc > 0;
        return Container(
          decoration: BoxDecoration(
            color: hasActive ? cat.color(notifier).withOpacity(0.14) : C.card,
            borderRadius: R.sm,
            border: Border.all(
              color: hasActive ? cat.color(notifier).withOpacity(0.35) : C.border,
              width: hasActive ? 1.5 : 0.5,
            ),
          ),
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(cat.icon, size: 22, color: hasActive ? cat.color(notifier) : C.t3),
            const SizedBox(height: 5),
            Text(cat.label, textAlign: TextAlign.center,
                style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600,
                    color: hasActive ? cat.color(notifier) : C.t3)),
            if (deEsaCat.isNotEmpty) Text('$enc/${deEsaCat.length}',
                style: TextStyle(fontSize: 8, color: hasActive ? cat.color(notifier) : C.t3)),
          ]),
        );
      }).toList(),
    );
...
extension CatColor on CatArtefacto {
  Color color(DispositivosNotifier n) => switch (this) {
...
// Tarjeta compacta de habitación para el dashboard
class _HabCardCompact extends StatefulWidget {
  ...
  const _HabCardCompact({required this.nombre, required this.dispositivos,
      required this.notifier, required this.onSnack});
...
class _HabCardCompactState extends State<_HabCardCompact> {
      ...
      InkWell(
        ...
        child: Padding(
          ...
          child: Row(children: [
            ...
            _Chip(
              enc: widget.dispositivos.where((d) => d.encendido).length,
              total: widget.dispositivos.length,
            ),
  ...
  IconData _habIcon(String n) {
    ...
    if (l.contains('jardín') || l.contains('jardin') || l.contains('patio'))
      ...
      child: Row(children: [
        Container(width: 34, height: 34,
          decoration: BoxDecoration(
            ...
            borderRadius: R.xs),
          child: Icon(d.cat.icon, size: 17,
              color: d.encendido ? d.tipo.color : C.t3)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(d.nombre, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: C.t1)),
          Text(d.ip, style: const TextStyle(fontSize: 11, color: C.t3)),
        ])),
        _busy
...
// ═══════════════════════════════════════════════════════════════
// PÁGINA: HABITACIONES  (control por sala)
// ═══════════════════════════════════════════════════════════════
...
class _HabPageState extends State<HabitacionesPage> {
  ...
  Widget build(BuildContext ctx) => AnimatedBuilder(
    ...
    builder: (_, __) {
      final n = widget.notifier;
      final habs = n.habitaciones.toList();
      ...
      return Scaffold(
        ...
        body: CustomScrollView(slivers: [
          ...
          // Grid de dispositivos de la habitación seleccionada
          if (items.isEmpty)
            SliverFillRemaining(
              ...
              sliver: SliverGrid(
                delegate: SliverChildBuilderDelegate(
                  (_, i) => _DevTile(
                    ...
                    onSnack: (msg, {error = false}) => showSnack(ctx, msg, error: error),
                  ),
...
// Tarjeta grande del dispositivo en la grid de habitaciones
...
class _DevTileState extends State<_DevTile> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext ctx) {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) => GestureDetector(
        ...
        onLongPress: () => _confirmDelete(ctx, d),
        child: AnimatedContainer(
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Icon(Icons.power_settings_new_rounded,
                          size: 16, color: d.encendido ? Colors.white : C.t2),
                    ),
                  ),
              ]),
              const Spacer(),
  ...
  void _confirmDelete(BuildContext ctx, Dispositivo d) => showDialog(
    context: ctx,
...
// ═══════════════════════════════════════════════════════════════
// PÁGINA: DISPOSITIVOS (lista administrable)
// ═══════════════════════════════════════════════════════════════
          ...
          if (filtered.isEmpty)
            SliverFillRemaining(
              child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 10),
                const Text('Sin resultados', style: TextStyle(color: C.t2)),
              ])),
            )
          else
            SliverPadding(
          ...
          child: Column(children: [
            ...
            Row(children: [
              ...
              if (d.ultimaAccion != null)
                _IChip(Icons.schedule_rounded, _rel(d.ultimaAccion!)),
              const SizedBox(width: 8),
              // Botón eliminar
              GestureDetector(
                onTap: () => showDialog(context: ctx, builder: (_) =>
                    _DeleteDialog(nombre: d.nombre, onConfirm: () {
      ...
      children: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: TipoD.values.map((t) {
          ...
          return GestureDetector(
            onTap: () => setState(() {
              _tipo = t;
              _cPuerto.text = t == TipoD.celular ? '8080' : '80';
            }),
            child: AnimatedContainer(
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(width: 5),
                Text(c.label, style: TextStyle(fontSize: 11,
                    fontWeight: FontWeight.w600,
                    color: sel ? col : C.t2)),
              ]),
            ),
